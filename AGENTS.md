...
| `src/data/rebrickable-logo.png` | Rebrickable wordmark (theme / card origin links, and the card-editor catalog search) |
...
| `src/js/hotkeys.js` | Keyboard shortcuts (print, backup / editor save, dialog primary action, search, new card, settings, themes, developer, about) and `aria-keyshortcuts` on the controls they activate |
...
| `src/js/telemetry.js` | Anonymous usage telemetry (opt-in, localStorage); public stats URL |
| `src/js/themes-data.js` | Loads the default-themes JSON, `logoSrc`, default accent, `parseRebrickableThemeId`, preset overlay merge/diff |
| `src/js/storage.js` | IndexedDB cards + custom themes / default-theme overlays, `.brickcard` import; `createRebrickableThemeId` |
| `src/js/sets-presets.js` | Loads `sets-presets.json`; `searchCatalogSets`, `searchCatalogThemes`, `catalogThemePathLabel`, `catalogSetImageUrl`, `cardDraftFromRebrickableSet`, `findThemeByRebrickableId`, `resolveOrCreateThemeFromRebrickable` (lazy; not imported at boot) |
| `src/js/set-search.js` | Catalog search combobox (`search-bar--suggest`, `bindSetSearch` / `bindThemeSearch`); card / theme editors + `#developer/search` demo |
| `src/js/rebrickable-ref.js` | Compact Rebrickable origin markup (hint then clickable logo, optional catalog path) for the theme editor and card editor |
| `src/js/roving-grid.js` | One tab stop per card or theme grid; arrows, Home / End, and print quantity `+` / `−` |
...
| `src/js/card-export.js` | Download (`data:` → blob file; other src → `target=_blank`); card photo names `brickcard-card-image-YYYY-MM-DD-…`; theme logos `brickcard-theme-logo-YYYY-MM-DD-…` |
...
| `src/js/welcome-dialog.js` | First-visit welcome modal (intro + LEGO disclaimer; off-local telemetry ask when no choice is stored, or local `?telemetry` preview); `import()` on home |
...
| `src/js/views/editor.js` | Card editor (Rebrickable set autocomplete; theme action stacks the theme manager / editor without changing the hash) |
| `src/js/views/themes.js` | Theme manager modal (mini-cards, search) |
| `src/js/views/theme-editor.js` | Create / edit a custom theme or customize a default theme (`#themes/edit/:id`) |
...
```js
{
  id: string,               // UUID (`createId()`); the same set can be added more than once
  ...
  imageDataUrl: string,     // photo (JPEG/PNG/SVG/WebP data URL, same-origin path, or https URL) — optional
...
- `logoSrc` (optional) — path from `src/` (e.g. `data/theme-logo-….png`) or an `https:` URL (draft tool); no logo → theme **name** / Brickcard mark on tiles; load failure → error brick (`ri-file-damage-fill`)
...
Default themes are editable in the app: saving a change stores a **sparse overlay** in IndexedDB with the **same `id` as the preset** (one customization per default theme). The customized theme moves to the custom list (`isBuiltin: false`); only differing fields are stored (empty `color` / `logoDataUrl` means cleared; an unchanged logo is not duplicated; `rebrickableThemeId` is stored when associating a catalog origin the JSON does not have). Identifier (readonly) and a short note appear only after a customization exists. **Remove customization** deletes the overlay and restores the default theme; cards keep `brickcardThemeId`. Custom themes from scratch have a UUID id (`createId()`), or `rebrickable-{rebrickableThemeId}` when created from the catalog (one Brickcard theme per catalog theme id). The card editor and theme editor keep Rebrickable refs on save (no freeform id field). `#themes/new` and `#themes/edit/:id` when `rebrickableThemeId` is empty: **Prefill from the catalog database** / **Associate with the catalog database** (`bindThemeSearch`, clickable Rebrickable logo only); picking fills **Name** (replace on create, empty-only on edit) and stores the catalog id on save; already-linked catalog ids are omitted from the list (Save also refuses a duplicate). `#themes/edit/:id`: if `rebrickableThemeId` is set, a compact origin (**This theme is referenced on** then the clickable Rebrickable logo → `https://rebrickable.com/sets/?theme={id}` `_blank`, then the catalog path **Parent > Theme** or the catalog name alone, 2 names max) is centered above the back preview; nothing if unset. Changing `brickcardThemeId` in the card editor does not update `rebrickableThemeId` / `rebrickableSetId` (origin metadata). `#themes/view/:id` redirects to `#themes/edit/:id`.

Developer tool `#developer/theme-presets`: isolated local copy (IndexedDB `brickcard-preset-draft`) to edit id/slug, name, colors (accent + secondary), logo and crop (`#developer/theme-presets/new`, `#developer/theme-presets/edit/:slug`), then **save** `themes-presets.json` + `theme-logo-{id}.{ext}` to drop into `data/` yourself (a remote `https:` logo is written as `logoSrc` and skipped by **Save logos**). Never reads/writes the `cards` / `themes` stores or settings. On first load (or after **Reset**): seed from the JSON. **General** success / errors (load, **Reset**, **Save themes-presets.json**, **Save logos**, save / delete in the editor) → `toast()`; Name / Identifier field validation → `form-error` under the input. Local collection reset does **not** touch this draft.

## Set catalog (`src/data/sets-presets.json`)

Compiled from the daily [Rebrickable downloads](https://rebrickable.com/downloads/) (CSV dumps, no API key, no image files). The app precaches the file offline. [`src/js/sets-presets.js`](src/js/sets-presets.js) loads it on demand (not at boot). Card-editor autocomplete (`#new-card`, and `#edit-card/:id` when `rebrickableSetId` is empty) uses `bindSetSearch`; theme-editor autocomplete (`#themes/new`, and `#themes/edit/:id` when `rebrickableThemeId` is empty) uses `bindThemeSearch`; `#developer/search` keeps both catalog suggest demos.
...
- `themeDisplayMap(themes)` — display copies only. A custom theme (not a default theme or its overlay) with a `rebrickableThemeId` fills empty `color`, `secondaryColor`, and logo (`logoDataUrl` + zoom / offsets as one block) from the nearest Brickcard ancestor on the catalog `parentId` chain (`findThemeByRebrickableId` order). Nothing is written to IndexedDB, import, or export. Default themes without `rebrickableThemeId` are not ancestors
...
- `catalogThemePathLabel(theme, themes)` — `"Parent > Theme"` when the immediate parent is named and in `themes`, else `"Theme"` (never more than 2 names)
- `searchCatalogSets(query, { limit })` — `queryNeedles` on set `id`, `name`, catalog theme name, and immediate parent theme name (leading `#` on a token stripped); space-separated tokens are AND; A–Z `name` then `id` (`localeCompare` + `getLocale()`); `items` include `themeName` (leaf) and `themePath` (display, 2 names max); `matchCount` is the full hit count; `items` is the full hit list unless `limit` is given; returns `generatedAt` and `setsImageUrl` from `meta`
- `searchCatalogThemes(query, { limit, exceptThemeId })` — `queryNeedles` on theme name and immediate parent name; roots first, then children; A–Z on `catalogThemePathLabel`; omits catalog ids already linked to another Brickcard theme (`exceptThemeId` = the theme being edited); `total` is the still-unlinked count
...
```js
{
  ...
  logoDataUrl: string,      // JPEG/PNG/SVG/WebP (data URL, same-origin path, or https URL), optional
  ...
  isBuiltin: boolean,       // default theme with no overlay; false = custom UUID or preset overlay
  rebrickableThemeId: number|null, // catalog theme id; null if unset; origin / matching only (overlays inherit from the JSON unless they set it)
  updatedAt: string         // ISO (custom / overlay); empty for unmodified default themes
}
```
...
2. else, for a custom theme with `rebrickableThemeId` (not a default theme or its overlay), the nearest Brickcard ancestor’s color on the catalog `parentId` chain (display only, `themeDisplayMap`; not stored)
3. else configured color (`card-design.js`)
4. else factory gray `DEFAULT_THEME_COLOR` (`#6e6e6e`)

Texts / icons / Brickcard logo (`resolveCardAccentFg`, `--card-accent-fg`):
1. `theme.secondaryColor` if valid hex
2. else that same ancestor’s `secondaryColor` if set (display only)
3. else auto contrast (`contrastText`): `#141414` only on a truly pale background (light gray / pastel); `#ffffff` on saturated hues (even bright ones) and dark backgrounds
Theme logo on the back and on theme tiles: the theme’s own logo, else that ancestor’s logo with its zoom and offsets (one block; display only). The Brickcard logo (back, and face with no photo) is an **inline SVG** (`fill="currentColor"` / `--card-accent-fg`) — not a CSS mask (Firefox rasterizes `mask-image` when printing).

## Persistence

- IndexedDB: `brickcard` **v2** — `cards` + `themes` stores (**custom UUID themes** and **sparse default-theme overlays** with the preset slug as `id`; unmodified default themes stay in `themes-presets.json`) (after local Reset: name `brickcard-<db-gen>`, key `brickcard:db-gen`); post-reset reload uses `?{timestamp}` (HTTP / SW cache), then the query is stripped at boot (`?_=` still recognized)
...
- Theme sort keys: `brickcard:themes-sort`, `brickcard:themes-sort-dir` (default `numCards` / `desc`; old `cardCount` still read); after **creating** a custom theme or **first save** of a default-theme customization: search cleared and sort reset to date modified descending (if ≥ 2 custom themes; else fall back to the default); after **edit** or **delete**: the grid is not rebuilt (mini-card updated or removed, or moved between Custom / Default; scroll / search / sort unchanged; counter recalculated; no custom left → custom section hidden); **Delete all custom themes** (if more than 2 custom, including overlays): custom section emptied, default themes restored; UUID-theme cards lose `brickcardThemeId`; overlay cards keep the default theme id; after **create** or **edit**: keyboard focus on the mini-card; closing the editor of an existing theme (Escape / close / Cancel / backdrop): same focus. Same idea for `#developer/theme-presets` (create: search cleared, date desc sort, scroll top, focus; edit: in-place tile and focus; closing the editor of an existing theme: focus; last theme → empty “No theme”)
...
- Telemetry key: `brickcard:telemetry` (`"1"` = on; missing / `"0"` = off); local Reset removes it; Settings checkbox off-local only, plus a local preview when the query `telemetry` is present and not `0` (`?telemetry`; kept across hash navigation; script injection and page views stay blocked on localhost / `127.0.0.1` / `[::1]`); Settings hint (under the checkbox, aligned with the label) states anonymous usage stats, that nothing identifies you and no IP is stored, and that usage data is public on `data.brickcard.org` (link to `TELEMETRY_PUBLIC_STATS_URL`); views = pathname + hash (`data-auto-track` off); stable Umami URL (English hashes, `#edit-card/:id` → `#edit-card`, `#themes/edit/:id` → `#themes/edit`; not other ids / slugs); title = `{locale} · ` + UI label (`getLocale()`, middle dot): home → `_t("Home")`, `#edit-card/…` → `_t("Edit card")`, `#themes/edit/…` → `_t("Edit theme")`; `#developer/…` = before the 2nd `|` (`page | section`, EN titles); other views = segment before `|` (not the SEO suffix); track at the end of `route()` (after title / overlay)
- Welcome key: `brickcard:welcome-seen` (`"1"` after the first-visit modal is closed); local Reset removes it; shown on home when missing (child dialog, no hash): intro + LEGO / trademark disclaimer; off-local (or local `?telemetry` preview), if `brickcard:telemetry` is not already `"0"` or `"1"`, also asks to enable anonymous stats (**Yes, enable telemetry** stores `"1"`, **No thanks** / Escape / close / backdrop stores `"0"`); an existing choice skips the ask (**Got it**, telemetry unchanged); leaving home without closing does not mark it seen; the preview still does not inject the script
- Card accent cascade: theme color → Rebrickable ancestor color (custom catalog child, display only) → configured color → `#6e6e6e`
- Texts / Brickcard logo cascade: theme `secondaryColor` → ancestor `secondaryColor` → auto contrast on the accent. Theme logo: own logo, else the ancestor’s logo and crop (display only; not stored)
- Screen: 2 px `box-shadow: 0 0 0 2px var(--ink)` hairline on `.card` / `.card-back` / `.theme-tile-face` (silhouette vs the UI background, follows `--card-radius`). Card and theme previews (tiles and editor) are not text-selectable and their images are not draggable. Print / A4 preview (white paper): 1 px `#000000` `border` on `.print-slot::after` if **Cut marks** On the front / On the back is checked (`.print-cut-mark-face` / `.print-cut-mark-back`; an `outline` on the slot sits underneath)
- Export (`.brickcard` file, JSON): `{ version: APP_VERSION, app: "brickcard", exportedAt, exportedFrom?, themes, cards, settings? }` — `version` = app SemVer (e.g. `"0.8.0"`) even with no structure change; `exportedAt` = ISO timestamp; `exportedFrom` = page origin + path (no hash / query) only when the host is a DNS name (omitted on localhost, IP, `.local` / `.localhost`; never read on import); `themes` = custom UUID themes plus **default-theme overlays** (diff vs the current JSON only; empty `color` / `logoDataUrl` kept when cleared; `isBuiltin` stripped); card / UUID-theme fields that are `null`, `""`, or a stored default are omitted (card crop `imageZoom` `1` / offsets `0`; theme logo crop `1` / `0` / `0`; import `normalize*` fills defaults for UUID themes); `settings.cardAppearance` (4 keys: border, corner / image radii, default color) omitted if not included — route `#backup` (`modal--md`; full: all cards, all custom themes even empty, appearance; custom: images & logos, appearance, themes with cards (alpha); no theme selected or no card: no export; without images / logos, photo, background, crop and logo are omitted from the file). Recap in the footer. Names: `brickcard-backup-YYYY-MM-DD-{full|custom}-{n}-card(s)[-{n}-theme(s)][-{n}-image(s)][-{n}-logo(s)].brickcard` (theme / image / logo segments omitted when 0)
- Import: route `#import` (`modal--md`); `.brickcard` file (or http(s) / relative URL, extension optional if the JSON is valid) loaded **in memory** then merge choices (images & logos, appearance, cards by theme — sections shown only if the file has them); IndexedDB write only on **Import** (merge, values replaced; local images / logos kept if the checkbox is unchecked); no wipe / replace of the collection; checks `app` / `version` / `cards` / `themes` (arrays, possibly empty); `app` = `brickcard`; rejects a SemVer `version` higher than `APP_VERSION`; structure migrations (`backup.js`, including old integer `version` 1–3) before normalize; ignores `isBuiltin: true` rows; **preset ids are overlays** (compacted against the current JSON); applies `settings.cardAppearance` if present and selected; file with no cards accepted if themes and/or settings remain; does not rewrite `themes-presets.json`; empty home: **Load a demo** tile (`ri-emotion-fill`) opens a `modal--sm` **Demo backup** (brick + “Loading”) that loads `data/backup-demo-jo.brickcard` and merges everything with no choice step, then closes the modal; toast **Demo loaded** (`ri-emotion-fill`, same recap as import)
...
Route overlays (one at a time, **swap** without tearing down the list): `#settings`, `#print`, `#backup`, `#import`, `#themes`, `#themes/new`, `#themes/edit/:id`, `#themes/view/:id`, `#page/:slug`, `#new-card`, `#edit-card/:id`, `#developer/…` (`#developer/theme-presets/new`, `#developer/theme-presets/edit/:slug`). Each overlay is loaded with `import()` on open (failure → toast, home stays usable). Child dialogs (confirmations including unsaved-close **Save?**, image / backup URL, home demo, first-visit welcome): no URL, second backdrop on top of the current view (on home: only backdrop). From the card editor, the theme manager / theme editor can stack the same way (no hash change; `#new-card` / `#edit-card/:id` stay); closing them returns to the card.

**Keyboard** (Ctrl on Linux / Windows, Cmd on macOS; no Alt, no Shift, no key repeat — except **/**, **?**, and **,** where Shift is allowed, and **Ctrl/Cmd+Alt+N**): **Ctrl/Cmd+P** opens `#print` (already open → start printing; blocked in a draft editor, `#import`, and a child dialog). **Ctrl/Cmd+S** opens `#backup` (already open → export; same block list) — in a card, theme, or preset editor it clicks **Save** (including the unsaved-close **Save?** button) and does not open the backup. **Ctrl/Cmd+Enter** clicks the front dialog’s primary footer button (`btn primary` in `.modal-footer`, not icon-only, not `danger`): Save, Import, Start printing, Load, New theme, welcome **Got it** / **Yes**. A disabled button is left as-is. **Ctrl/Cmd+Alt+N**, or **N** when the focus is not in a text field, opens `#new-card` (same block list as print: draft editor, `#import`, child dialog). **Ctrl/Cmd+N** without Alt is reserved by Chrome and Firefox for a new window and is not delivered to the page (still handled if a browser does send it). **Ctrl/Cmd+,**, or **,** outside a text field, opens `#settings`. **T** outside a text field opens `#themes`. **D** outside a text field opens `#developer`. **?** outside a text field opens `#page/about`. Same block list as a new card (a hash change does not ask **Save?**). Already on that route: the shortcut does nothing. **Tab** from outside the top bar skips it and lands on the card list (or the next control after the bar). Inside the top bar, **Tab** / **Shift+Tab** keep the usual order, including the logo (`#page/about`). A card grid and each theme grid (**Custom** / **Default**) is one **Tab** stop (`role="grid"`): **Arrow keys** move between tiles, **Home** / **End** jump to the first and last visible tile, **Enter** / **Space** opens it, **+** / **−** changes the focused card’s print quantity (the quantity buttons stay mouse-clickable and out of the tab order). **Ctrl/Cmd+F**, or **/** outside a text field, focuses the search bar of the front view (home, themes, settings, developer, catalog suggest) and selects its text. No search bar in that view → **Ctrl/Cmd+F** stays the browser find (the home bar behind a modal does not count). **Escape** in a search field that has text clears it when its suggestion list or sort menu is closed; the next Escape closes the dialog.

- (no hash) home (empty “Welcome ;)”, list, or search with no results “Oops!”); `#` and `#/` cleaned
- `#new-card` `#edit-card/:id` card editor (modal); face + back preview; < 550px: one side visible (face first), click / Enter / Space to flip; **Prefill from the catalog database** (`#new-card`) / **Associate with the catalog database** (`#edit-card/:id` without origin) (`bindSetSearch`, clickable Rebrickable logo only — no link on the label) when `rebrickableSetId` is empty (right column on wide screens; above the previews at ≤840px; at < 550px the label, Rebrickable logo, and search field are centered, field full width, catalog date hidden) — new card: picking a set replaces all known fields (including the remote photo URL) without confirm; edit without origin: fill empty fields only; a matching Brickcard theme is selected if one exists, else an in-memory `rebrickable-{themeId}` option (name only) until **Save** (`resolveOrCreateThemeFromRebrickable`; **Manage themes** while pending); catalog-created card ids stay unique (`createId()`); a saved `rebrickableSetId` hides the search and shows the origin block above the previews (**This set is referenced on** then the clickable Rebrickable logo → `https://rebrickable.com/sets/{id}/`, then the catalog path **Parent > Theme** or the catalog theme name alone, 2 names max, with `ri-palette-fill`, then the catalog set name); theme field: full-width select + primary **Customize the theme** / **Manage themes** (`ri-palette-fill`); a selected (persisted) theme opens the theme editor stacked, no theme or a pending catalog theme opens the theme manager stacked (hash unchanged); close / Escape on the stacked overlay returns to the card; the select and face / back preview refresh from IndexedDB (deleted theme → **No theme**; a newly created theme is listed, not auto-selected); after create: search cleared, date desc sort, focus on the tile; after edit: tile updated and focused (scroll / sort / search kept); close without saving (`#edit-card/:id`, Escape / close / Cancel / backdrop): focus on the edited tile; if a persistable field changed, Escape / close / backdrop show **Save?** first (`confirmUnsavedClose`: **Close without saving** / **Cancel** keeps the editor / **Save** same persist as the footer); footer **Cancel** and an unchanged form close immediately; after delete: tile removed (scroll / sort / search kept)
- `#themes` theme manager (`lg` modal); custom and **default** tiles focusable / clickable (both open the editor); `#themes/new` `#themes/edit/:id` theme editor (modal over the list; close → `#themes`); `#themes/view/:id` redirects to edit; a default theme with no overlay: no identifier, no note, no delete (Save creates an overlay only if a value differs; unchanged Save closes with no toast); a saved overlay: readonly identifier, note **Customization of the default “%(name)s” theme.** (preset name), **Remove customization** (cards stay on the default theme); UUID custom: Delete as before (cards lose the theme); overlay save toasts **Customization saved** (UUID custom: **Theme saved**); first overlay save treated as create (search cleared, date desc, tile moves to Custom); after edit: tile updated and focused (scroll / search / sort kept); close without saving (`#themes/edit/:id`, Escape / close / Cancel / backdrop): focus on the mini-card; if a persistable field changed: Escape / close / backdrop show **Save?** first (`confirmUnsavedClose`; **Save** creates or updates); footer **Cancel** closes immediately; after delete / remove customization: tile removed or restored to Default; footer: **New theme** on the right; if more than 2 custom themes (UUID + overlays), **Delete all custom themes** (`ri-delete-bin-2-fill`, danger) on the left (confirmation; UUID cards lose the theme, overlay cards keep the default); `#themes/new` and `#themes/edit/:id` without `rebrickableThemeId`: **Prefill from the catalog database** / **Associate with the catalog database** (`bindThemeSearch`, logo link only; right column on wide screens, above the preview at ≤840px; at < 550px the label, logo, and search field are centered like the card editor); `#themes/edit/:id` with `rebrickableThemeId`: **This theme is referenced on** then the clickable Rebrickable logo, then the catalog path **Parent > Theme** (or the catalog name alone, 2 names max), centered above the back preview (not editable)
- `#settings` settings (`md` modal); Application section: **Language** first (`select` `DE · Deutsch` / `EN · English` / …, ISO code ASC, `brickcard:ui-locale`); search bar (`search-bar--input-only`): section titles, tiles (`title` / `desc` / `href`), shortcut rows (name / hint / keys), `form-label` / `form-hint`; case- and accent-insensitive; if a section title matches, the whole section; **Oops!** if no results; **Keyboard shortcuts** section after **Manage your collection** lists every shortcut (`kbd`: flat fill `--ink`, text `--bg`, no border or shadow) in menu order: New card, Save, Print, Search, Settings, Themes, Developer space, About, Main button, Close, then moving in a card or theme grid (arrow keys, Home, End) and the focused card’s print quantity (`+` / `−`); `aria-keyshortcuts` on the controls they activate (new card, print, save, settings, themes, developer, about, search, dialog main button)
- `#print` print settings (`md` modal); nothing to print → message instead of options; **Ctrl/Cmd+P** shortcut (outside card / theme / presets editor, outside `#import` and outside a child dialog); already open → starts printing; **Ctrl/Cmd+Enter** starts printing when the footer button is shown
- `#backup` collection backup (`md` modal); full or custom; recap in the footer (cards, themes, settings, size; empty: **No cards to save!**); close → home; **Ctrl/Cmd+S** shortcut (outside card / theme / presets editor, outside `#import` and outside a child dialog); already open → starts the export; **Ctrl/Cmd+Enter** starts the export; successful export → closes the modal
...
Same field order. Control: text `input.form-control` in a `form-color` wrapper, with a swatch on the left (`input[type=color]`) and a clear button (`ri-close-circle-fill`) overlaid inside the field. Clear visible only when there is a value (may be omitted / disabled); not focusable (`tabindex="-1"`). Swatch: only if valid hex; else the field default color (`fallback` / `fallbackColor`), else a transparent checkerboard. Placeholder = fallback (updated by `setValue`). `disabled` option: hex + swatch frozen, clear hidden. Module: `form-color.js`. Applied: settings · themes · image field (background). Gallery: `#developer/colors`.
...
- **Empty** — hint “Load a new image to preview and crop it” (`form-hint`) + **From my files** (`btn primary`, `ri-file-line`, hidden `<input type="file">`) and **From a URL** (`btn secondary sm`, `ri-link`). URL → child `modal--sm` with no route (title “Load from a URL” + `ri-link`, URL field with `ri-cloud-fill`, `form-error` under the input; vertical radios: **Import the image into the collection** (default, hint: always displayed even offline; `fetchImageAsFile` then `processFile`, URL not kept) / **Use the remote URL** (hint: requires Internet; `Image()` probe, no fetch; store the `https:` URL in `imageDataUrl` / `logoDataUrl`); only `https:` (else “Only HTTPS image URLs are allowed.”); loading: brick + “Loading” under the field); footer on the right **Cancel** `secondary sm` + **Load** `primary` (`ri-upload-fill`, like Import); close only if the load succeeds (Escape / backdrop / X = dismiss).
- **Image** — **Image background** field (`form-color`, no hint) then `.form-image-crop` preview (`tabindex="0"`). Overlays: 3 centered badges (`btn primary sm`, appearance only: zoom `%`, signed alignment `%`; `ri-zoom-in-fill` / `ri-align-item-horizontal-center-fill` / `ri-align-item-vertical-center-fill`); reset `btn ghost sm icon-only` (`ri-close-circle-fill`) top-right if crop ≠ 100% / 0 / 0; **Delete** (left, `ri-delete-bin-2-fill`) and **Download** (right, `ri-download-fill`, like Save the collection) `btn primary sm` (delete: `confirmDialog`; successful `data:` save: toast **Image saved**; any other src **Download** opens `target=_blank`; cards: name `brickcard-card-image-YYYY-MM-DD-{ref-slug}-{title-slug}.{ext}` — slugged ref and/or title; without both: card id; theme logos: `brickcard-theme-logo-YYYY-MM-DD-{name-slug}.{ext}`; without a name: theme id). Failed load (`onerror`): keep the src, hide the `<img>`, show a printable 404-style brick (`.brick--image-error`, `ri-file-damage-fill`; black / white via `contrastText` on the frame / theme accent) and the centered caption **Remote image unavailable** (same contrast); crop badges off. Preview tab order: reset (if visible) → Download → Delete. Crop when focused (drag / wheel / arrows / `+` `−`). Ratio: `--form-image-aspect` (default `1 / 1`). Preview background = field color, live.
...
Applied: card editor · theme editor · default-themes tool (`#developer/theme-presets`). Gallery: `#developer/images`.
...
| Trail | `search-bar-trail` (absolute, right) — visible only if ≥ 2 items (`[hidden]` otherwise); catalog suggest shows count ` · ` catalog date (date hidden at < 550px, or first when the bar is too narrow); `--search-trail-pad` follows the trail width (`search-bar--trail-below` if the count still does not fit) |
...
| Suggest | catalog combobox (`bindSetSearch` / `bindThemeSearch` in `set-search.js`): list opens after 1 character (configurable); first 25 hits then 25 more on scroll (optional `maxResults` cap); set search matches `id` / `name` / catalog theme / immediate parent theme (AND on spaces); `form-select-list` + `form-select-option--multiline` (square set photo on the left via `catalogSetImageUrl` / `meta.setsImageUrl`, centered `object-fit: cover` in `.form-select-option-media`; `ri-hashtag` + id, year/pieces/figurines badges, theme path `Parent > Theme` (2 names max) + `ri-palette-fill`, set name; query hits wrapped in `<b>`); theme search matches theme name / parent name, one path line with `ri-palette-fill`, roots then children A–Z, already-linked catalog ids omitted; blur hides the list, focus reopens it (scroll reset to the top on input and focus) |
...
Applied: list topbar · themes modal (results + sort: `numCards`, title, date modified if ≥ 2 custom themes — default themes not involved; default `numCards` descending) · `#developer` home and `#settings` (`search-bar--input-only`, no results or sort) · card editor **Prefill from the catalog database** / **Associate with the catalog database**, theme editor the same pair, and `#developer/search` set + theme catalog demos (`search-bar--suggest`, `sets-presets.json`). Matching: `queryNeedles` / `matchesNeedles` (`includes-ci.js`) — space-separated tokens are AND (each token must match at least one field; case and accents ignored; leading `#` on a token stripped). Settings / developer home: if every token is in the section title, the whole section is shown; otherwise each field or tile matches if every token is in the section title or that item. Gallery: `#developer/search`.
...
| Dialog | `h1.view-title` (`aria-labelledby`) — **short** title + **one** Remix icon max on the left if the trigger has one (`modalTitleMarkup`): icon on the left vertically centered, title beside it (several lines if needed, like toasts); card / theme edit: `ri-pencil-fill`; confirmations: a bit longer, with the subject; galleries / development help / templates: section (`#developer` link, optional icon) + `ri-arrow-right-wide-fill` + title (no tile icon); `#page/about`: title only (from the markdown `#`, no icon or version) | `h2.section-title` |
...
- **Git — commits**: commit as soon as an intent (feature or fix) is **done**, or **before** starting another. One commit = one intent (a revert = one thing). Do not dump a whole session. Never commit `.local/` or `.cursor/`. If a higher-priority rule blocks committing without an explicit ask: **tell the user now** (what to commit and why); do not keep piling uncommitted work. Push and version bump stay on request (see below).
- **Git — version, tag, and push**: **only on an explicit request**. Bump `APP_VERSION` (and the cache `?v=` in `index.html` and `404.html`: CSS and `app.js` / `version.js` import map, `CACHE` in `service-worker.js`, and a dated `CHANGELOG.md` entry) only when asked. Between versions, record **app code** (`src/`) changes under `## [Unreleased]` **in English** (not README, CONTRIBUTING, CI, GitHub templates, Ko-fi). If the number is not given, ask (patch / minor / major) — do not pick. Accepted **without confirmation**: only the next patch (`0.8.0` → `0.8.1`), next minor (`0.8.0` → `0.9.0`), or next major (`0.8.0` → `1.0.0`), from the current `APP_VERSION`. Anything else (jump `0.7.1` → `0.9.0`, downgrade, same number, invalid SemVer, `vX.Y.Z` tag already present): **stop and ask** before bump / tag / push. Bump OK → `🔖 chore: bump to X.Y.Z` commit + annotated `vX.Y.Z` tag. Push (commits **and** tags) only if asked. Pushing the `vX.Y.Z` tag publishes the GitHub Release (`release.yml` workflow, native zip/tar.gz); identical commit subjects are listed once (with a count, e.g. daily `🗃️ chore: refresh sets-presets.json from Rebrickable`); `chore: bump to X.Y.Z` and merge commits are omitted; do not call `gh release create` locally.
- **Git — messages**: `<gitmoji> <type>: <why>` (English); optional body. Types stay `feat` / `fix` / `docs` / `chore` (`feat` = new capability, `fix` = bugfix, `docs` = AGENTS / README, `chore` = version bump, CI, assets). No scope (`feat(print):`), no other type. Example: `✨ feat: open the main views from the keyboard`. [Conventional Commits](https://www.conventionalcommits.org/en/v1.0.0/) and [gitmoji](https://gitmoji.dev). Changelog bullets (app code only, under `## [Unreleased]`) start with that gitmoji. Sections: `✨ Added` (`feat`), `🐛 Fixed` (`fix`), `🔥 Removed` (a behavior taken out), `♻️ Changed` (anything else a user can see; the bullet uses the gitmoji that fits, e.g. `💄` UI, `⚡️` performance, `♿` accessibility, `♻️` refactor). `docs` and an internal `chore` (bump, catalog refresh) stay out of the changelog. `release.yml` drops a leading gitmoji, then groups `feat` / `fix` / `docs` / `chore` as before.
