...
- Clé réglages impression : `brickcard:print-settings` (`{ printGrid: 1–10, cardSort: "legoSetRef"|"title"|"releaseYear"|"pieceCount"|"figurineCount"|"updatedAt", cardSidesToPrint: "faceAndBack"|"faceOnly"|"backOnly", sheetRectoVerso: "alternate"|"grouped" }`, défaut `3` / `legoSetRef` / `faceAndBack` / `alternate` ; tri toujours ASC, indépendant de `brickcard:list-sort`)
...
États vides (`section.empty-view` / `.empty-view-body`, helper `emptyViewMarkup` dans `empty-view.js`)&nbsp;: titre + texte + tuiles centrés dans `#main` ou le `modal-body` ; brique CSS collée au-dessus (hors flux). Accueil sans carte&nbsp;: «&nbsp;Bienvenue ;)&nbsp;» + tuiles Nouvelle carte / Importer une sauvegarde / Charger une démonstration (`welcomeViewMarkup` ; galerie `#developer/welcome`, tuiles import et démo inertes) ; sur grand écran le bloc remonte d’une hauteur de header (header visuellement vide) ; sur petite hauteur (ex. téléphone paysage) le bloc s’aligne en haut, `#main` défile, et une réserve compte la brique hors flux. Recherche sans résultat (cartes, thèmes, paramètres, accueil espace développeur)&nbsp;: «&nbsp;Oups&nbsp;!&nbsp;». Premier affichage&nbsp;: «&nbsp;Chargement...&nbsp;» jusqu’à cartes + thèmes prêts ; échec de boot → message rouge (`#boot-error`), animation arrêtée, `ri-error-warning-line` au centre de la brique (`loadingViewMarkup` ; galerie `#developer/loading` : en cours + erreur).
