...
| `src/data/page-{{slug}}.md` | Pages Markdown en modale (`#page/:slug`, ex. `page-about.md`) ; `# Titre` → titre du dialog ; HTML brut comme GitHub (pages `data/` de confiance) ; `#page/about` : marque (logo + nom + version, comme le header) injectée en tête ; Ko-fi en markdown en pied (comme le README) |
...
- Clé télémétrie : `brickcard:telemetry` (`"1"` / absente = coché, défaut ; `"0"` = décoché) ; Reset local la retire ; case Paramètres et script hors local seulement ; vues = pathname + hash (`data-auto-track` off) ; titre Umami = segment avant `|` (pas le suffixe SEO) ; `#developer/…` = avant le 2ᵉ `|` (`page | section`) ; accueil → **Accueil** ; `#edit-card/:id` → `#edit-card` / **Modifier la carte**, `#themes/edit/:id` → `#themes/edit` / **Modifier le thème** (pas les autres ids / slugs)
...
- `#page/:slug` page Markdown (`data/page-{{slug}}.md`, ex. `#page/about`) ; about : injection JS (marque header en tête) ; Ko-fi en markdown en pied
