...
| Suggest | catalog combobox (`bindSetSearch` in `set-search.js`): list opens after 1 character (configurable); max 50 results (configurable); matches `id` / `name` / catalog theme (AND on spaces); `form-select-list` + `form-select-option--multiline` (square set photo on the left via `catalogSetImageUrl` / `meta.setsImageUrl`, centered `object-fit: cover` in `.form-select-option-media`; `ri-hashtag` + id, year/pieces/figurines badges, theme + `ri-palette-fill`, set name; query hits wrapped in `<b>`); blur hides the list, focus reopens it (scroll reset to the top on input and focus) |
