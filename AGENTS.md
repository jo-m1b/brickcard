...
| `src/js/storage.js` | IndexedDB cards + custom themes / default-theme overlays, `.brickcard` import; `createRebrickableCardId` / `createRebrickableThemeId` |
...
| `src/js/views/editor.js` | Card editor (theme action stacks the theme manager / editor without changing the hash) |
...
- `findThemeByRebrickableId(themeId)` — default theme first (including a saved customization of that preset), then a custom UUID; match on the `rebrickableThemeId` field (not the Brickcard id)
