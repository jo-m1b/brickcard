...
| `src/js/set-search.js` | Set-catalog search combobox (`search-bar--suggest`, `bindSetSearch`); card editor + `#developer/search` demo |
| `src/js/rebrickable-ref.js` | Compact Rebrickable origin markup (hint then clickable logo) for the theme editor and card editor |
...
```js
{
  ...
  rebrickableThemeId: number|null, // catalog theme id; null if unset; origin / matching only (overlays inherit from the JSON)
...
| Suggest | catalog combobox (`bindSetSearch` in `set-search.js`): list opens after 1 character (configurable); first 25 hits then 25 more on scroll (optional `maxResults` cap); matches `id` / `name` / catalog theme (AND on spaces); `form-select-list` + `form-select-option--multiline` (square set photo on the left via `catalogSetImageUrl` / `meta.setsImageUrl`, centered `object-fit: cover` in `.form-select-option-media`; `ri-hashtag` + id, year/pieces/figurines badges, theme + `ri-palette-fill`, set name; query hits wrapped in `<b>`); blur hides the list, focus reopens it (scroll reset to the top on input and focus) |
