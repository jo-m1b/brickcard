...
Route overlays (one at a time, **swap** without tearing down the list): `#settings`, `#print`, `#backup`, `#import`, `#themes`, `#themes/new`, `#themes/edit/:id`, `#themes/view/:id`, `#page/:slug`, `#new-card`, `#edit-card/:id`, `#developer/…` (`#developer/theme-presets/new`, `#developer/theme-presets/edit/:slug`). Each overlay is loaded with `import()` on open (failure → toast, home stays usable). Child dialogs (confirmations including unsaved-close **Save?**, image / backup URL, home demo): no URL, second backdrop on top of the current view (on home: only backdrop). From the card editor, the theme manager / theme editor can stack the same way (no hash change; `#new-card` / `#edit-card/:id` stay); closing them returns to the card.
