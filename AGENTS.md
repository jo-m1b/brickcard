...
- Clé télémétrie : `brickcard:telemetry` (`"1"` / absente = coché, défaut ; `"0"` = décoché) ; Reset local la retire ; case Paramètres et script hors local seulement ; vues = pathname + hash (`data-auto-track` off) ; titre Umami = segment avant `|` (pas le suffixe SEO) ; accueil → **Accueil** ; `#edit-card/:id` → `#edit-card` / **Modifier la carte**, `#themes/edit/:id` → `#themes/edit` / **Modifier le thème** (pas les autres ids / slugs)
