...
/**
 * Brickcard mark (inline SVG, `fill=currentColor` / `--card-accent-fg`).
 * Not a CSS mask: Firefox rasterizes mask-image when printing (blurry logo).
...
export function applyImageTransform(imgEl, boxEl, crop) {
  ...
  // clientWidth/Height = layout box (ignores a possible parent CSS scale)
...
/**
 * Zoom / offset of a theme logo (`--logo-zoom`, `--logo-offset-x/y`).
 * `--logo-zoom`: 1 = 75% of the card width.
...
/** Brand markup (logo + name), face (under the image) and back. */
...
function applyThemeLogo(root, legoTheme) {
  ...
  // Crop known immediately (the image may still be hidden).
  root.dataset.logoZoom = String(legoTheme?.logoZoom || 1);
  root.dataset.logoOffsetX = String(legoTheme?.logoOffsetX || 0);
  root.dataset.logoOffsetY = String(legoTheme?.logoOffsetY || 0);
  applyThemeLogoTransform(themeLogo, themeWrap, legoTheme);
...
/**
 * Fill the title (line breaks via `\n`).
...
export function fillCardFace(root, card, legoTheme) {
  ...
  if (photo) {
    // Area background = card accent (visible under the photo’s rounded corners).
...
/**
 * Single preview (list, editor…): same markup + same poker size.
...
/**
 * Update an already-mounted preview (same logic as the editor).
...
/**
 * @param {HTMLElement} root `.card-back` element
 ...
 * @param {Partial<import("./storage.js").Card>} card
 * @param {{ legoTheme?: import("./themes-data.js").LegoTheme|null }} [opts]
 * @returns {HTMLElement} the `.card-back` element
 */
...
/**
 * @param {HTMLElement} cardEl `.card-back` element
 * @param {Partial<import("./storage.js").Card>} card
 * @param {{ legoTheme?: import("./themes-data.js").LegoTheme|null }} [opts]
 */
